...
export default function EditAccident() {
    ...
    const formik = useFormik({
        initialValues: {
            date: moment(state.existingData?.date,).format('yyyy-MM-DD'),
            plateNumber: state.existingData?.plateNumber,
            location: state.existingData?.location, casualties: state.existingData?.casualties,
            degree: state.existingData?.degree, details: state.existingData?.details,
        },
        ...
        onSubmit: async value => {
            const body = {
                date: value?.date, plateNumber: value?.plateNumber, location: value?.location,
                casualties: value?.casualties, degree: value?.degree, details: value?.details
            }
    ...
    return (
        <Box sx={{ maxWidth: '60%', mx: 'auto' }}>
            ...
            {state.existingData && <form onSubmit={formik.handleSubmit}>
                <Box sx={{ mb: 3, mt: 1 }}>
                    {label({ label: 'Date' })}
                    {textField({
                ...
                <Box>
                    ...
                    {textField({
                        nameValue: nameValue, id: 'casualties', type: 'number',
